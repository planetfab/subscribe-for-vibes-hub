...
- **`meta._yoast_wpseo_metadesc`** — set to `meta_description` if present, hard-truncated to 155 chars at a word boundary via `truncateMeta()` (exported from `src/claude.js`, reused in `src/publishers/wordpress.js`) immediately before send. This runs unconditionally regardless of source — Claude generation, Research & Enrich, or a manual edit in the modal — since relying on the system prompt's 155-char instruction alone has already proven unreliable (confirmed exceeding it by a few chars in production, August 6 2026).
